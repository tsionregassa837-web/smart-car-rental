...
from .forms import ExtraChargeForm, FleetVehicleForm, RentalInspectionForm
from .helpers import validate_booking_dates
from .models import Booking, Car, Contact, ExtraCharge, RentalInspection
...
def fleet_management(request):
    ...
    Operational fleet status comes from Car.fleet_status:
        - Available
        - Maintenance
        - Inactive

    Reservation/rental status is calculated from bookings:
        - Reserved = Approved bookings
        - Rented = Active bookings

    Reserved and Rented are NEVER stored in Car.fleet_status.
    """
    ...
    # IMPORTANT:
    #
    # Reserved and Rented are calculated states.
    # We do NOT filter using Car.fleet_status.
    # --------------------------------------------------------
    ...
    elif status in valid_operational_statuses:
    ...
    for car in cars:
        ...
        # Never allow booking counts to exceed
        # the physical fleet quantity.
        ...
        maintenance_count = max(
            int(car.maintenance_count or 0),
            0
        )
        ...
        if car.fleet_status == "Inactive":
            ...
            # An inactive fleet is not operational.
            maintenance_count = 0
            reserved_count = 0
            rented_count = 0
        ...
        elif car.fleet_status == "Maintenance":

            # If the entire fleet is under maintenance,
            # all units are maintenance units.
        ...
        # ----------------------------------------------------
        #
        # Only operational units participate here.
        #
        # total
        #   - maintenance
        #   - inactive
        ...
        #   = available
        #
        # This guarantees that the displayed categories
        # never exceed the physical fleet quantity.
        # ----------------------------------------------------
        ...
        if category_total > total_quantity:
        ...
        # ----------------------------------------------------
        #
        # This is useful for the template.
        #
        # Operational status has priority:
        #
        # Inactive
        # Maintenance
        #
        # Otherwise booking state:
        #
        # Rented
        # Reserved
        # Available
        ...
        if car.fleet_status == "Inactive":
            ...
            calculated_status = "Maintenance"

        elif rented_count > 0:

            calculated_status = "Rented"

        elif reserved_count > 0:

            calculated_status = "Reserved"
...
def fleet_vehicle_edit(request, car_id):
    ...
    Fleet status may only be:
        Available
        Maintenance
        Inactive

    Reserved and Rented are calculated from bookings and
    cannot be manually selected as fleet_status.

    Payment logic is not modified.
    """
...
def fleet_vehicle_add(request):
    ...
    Reserved and Rented are not selectable because they are
    calculated from booking data.
    """
...
def fleet_vehicle_detail(request, car_id):
    ...
    Fleet operational status remains separate.
    """
...
def fleet_vehicle_archive(request, car_id):
    """
    Archive a fleet vehicle by marking it Inactive.

    Vehicles with active rentals or approved reservations
    cannot be archived.

    Historical booking records are preserved.
    """
...
def fleet_vehicle_delete(request, car_id):
    ...
    Existing URLs/templates using fleet_vehicle_delete
    continue to work while the project moves to the
    archive terminology.
    """
...
def update_booking_status(request, booking_id, status):
    ...
    with transaction.atomic():
        ...
        # Serialize approvals for the same fleet row so two staff
        # requests cannot approve the last available unit at once.
        if booking.car_id:
            booking.car = Car.objects.select_for_update().get(pk=booking.car_id)
...
@staff_member_required
@require_POST
def rental_action(request, booking_id, action):
    """
    Backward-compatible quick rental actions.
    ...
    Detailed pickup/return inspections are handled by
    rental_operations().
    """
    with transaction.atomic():
        booking = get_object_or_404(
        ...
        if booking.car_id:
            booking.car = Car.objects.select_for_update().get(
                pk=booking.car_id
            )
        ...
        if action == "start":
            ...
            if booking.status != "Approved":
                messages.error(
                    request,
                    f"Booking #{booking.id} cannot start because "
            ...
            if booking.payment_status != "Paid":
                messages.error(
                    request,
                    "Payment must be completed before the rental can start.",
                )
                return redirect("booking_management")
            ...
            if booking.document_status != "Verified":
                messages.error(
                    request,
                    "Customer documents must be verified before rental pickup.",
                )
                return redirect("booking_management")

            if (
                not booking.car
                or booking.car.fleet_status in {"Maintenance", "Inactive"}
            ):
                messages.error(
                    request,
                    "The assigned vehicle is not operationally available.",
                )
                return redirect("booking_management")
            ...
            messages.success(
                request,
                f"Rental for booking #{booking.id} started. "
                "Use Rental Operations to record inspection details.",
            )
        ...
        # ============================================================
            ...
            if booking.status != "Active":
                messages.error(
                    request,
                    f"Booking #{booking.id} cannot be completed because "
                    f"its current status is {booking.status}.",
                )
                return redirect("booking_management")
            ...
            # The detailed return workflow is handled by
            # rental_operations(action="return").
            #
            # This quick action exists for backward compatibility.
            # Booking.save() normally requires return_mileage when
            # completing a rental, so use a direct UPDATE here rather
            # than fabricating a mileage value.
            ...
            messages.success(
                request,
                f"Rental for booking #{booking.id} completed successfully.",
            )
...
@staff_member_required
def rental_operations(request, booking_id):
    """
    Staff workspace for pickup/return inspection,
    damage and extra charges.

    Return mileage must never be lower than pickup mileage.
    Invalid return data is rendered back to the form with
    HTTP 200 and does not complete the rental.
    """
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            if action == "pickup":
                ...
                if booking.status != "Approved":
                    messages.error(
                        request,
                        "Only approved bookings can be picked up.",
                    )
                ...
                elif booking.document_status != "Verified":
                    messages.error(
                        request,
                        "Documents must be verified before pickup.",
                    )

                elif (
                    not booking.car
                    or booking.car.fleet_status
                ...
                elif RentalInspection.objects.filter(
                    booking=booking,
                    inspection_type="Pickup",
                ).exists():
                    messages.error(
                        request,
                        "A pickup inspection already exists for this booking.",
                    )

                else:
                    form = RentalInspectionForm(request.POST)
                    ...
                    if form.is_valid():
                        ...
                        try:
                            ...
                            context = {
                                "booking": booking,
                                "pickup_form": form,
                                "return_form": RentalInspectionForm(),
                                "charge_form": ExtraChargeForm(
                                    prefix="charge"
                                ),
                                "inspections": booking.inspections.all(),
                                "extra_charges": booking.extra_charges.all(),
                            }

                            return render(
                                request,
                                "core/rental_operations.html",
                                context,
                            )

                        booking.pickup_mileage = inspection.mileage
                        booking.pickup_fuel_level = inspection.fuel_level
                        booking.pickup_notes = inspection.notes
                        booking.pickup_completed = timezone.now()
                        booking.status = "Active"
                        ...
                        messages.success(
                            request,
                            "Pickup completed and inspection recorded.",
                        )
            ...
            # ========================================================
                ...
                if booking.status != "Active":
                    ...
                    messages.error(
                        request,
                        "A return inspection already exists for this booking.",
                    )

                else:
                    inspection_form = RentalInspectionForm(
                        request.POST
                    )

                    charge_data_present = any(
                    ...
                    inspection_valid = inspection_form.is_valid()

                    # ------------------------------------------------
                    # VALIDATE OPTIONAL EXTRA CHARGE
                    # ------------------------------------------------

                    charge_valid = (
                        not charge_data_present
                        or charge_form.is_valid()
                    )

                    # ------------------------------------------------
                    # RETURN MILEAGE VALIDATION
                    #
                    # Return mileage cannot be lower than pickup
                    # mileage.
                    # ------------------------------------------------

                    if (
                        inspection_valid
                        and booking.pickup_mileage is not None
                    ):
                        return_mileage = (
                            inspection_form.cleaned_data.get(
                                "mileage"
                            )
                        )

                        if (
                            return_mileage is not None
                            and return_mileage
                            < booking.pickup_mileage
                        ):
                            inspection_form.add_error(
                                "mileage",
                                (
                                    "Return mileage cannot be "
                                    "less than pickup mileage "
                                    f"({booking.pickup_mileage})."
                                ),
                            )
                    ...
                    if not inspection_valid or not charge_valid:

                        context = {
                            "booking": booking,
                            "pickup_form": RentalInspectionForm(),
                            "return_form": inspection_form,
                    ...
                    inspection.booking = booking
                    inspection.inspection_type = "Return"
                    inspection.created_by = request.user
                    ...
                    booking.return_mileage = inspection.mileage
                    booking.return_fuel_level = inspection.fuel_level
                    booking.return_notes = inspection.notes
                    ...
                    if (
                        charge_data_present
                        and charge_form.cleaned_data.get("amount")
                    ):
                    ...
                    messages.success(
                        request,
                        "Return inspection recorded and rental completed.",
                    )
    ...
    context = {
        ...
        "inspections": booking.inspections.select_related(
            "created_by"
        ).all(),
        "extra_charges": booking.extra_charges.select_related(
            "created_by"
        ).all(),
    }
