...
class Car(models.Model):
    ...
    def update_fleet_status(self):
        """
        Keep the fleet's operational status independent from
        booking/rental status.

        Available, Maintenance and Inactive are operational
        fleet states controlled by staff.

        Reserved and Rented are calculated from bookings and
        must never be stored as Car.fleet_status.
        """
...
class Booking(models.Model):
    ...
    pickup_fuel_level = models.CharField(
        max_length=30,
        blank=True,
        null=True
    )
    ...
    return_fuel_level = models.CharField(
        max_length=30,
        blank=True,
        null=True
    )
...
#
        ...
        if self.status == "Completed":
            ...
            if self.return_mileage is None:
                errors["return_mileage"] = (
                    "Return mileage is required before completing a rental."
                )
...
# =================================================
    ...
    # -------------------------------------------------
        ...
        if self.return_mileage is not None and self.pickup_mileage is not None:
            if self.return_mileage < self.pickup_mileage:
                errors["return_mileage"] = (
                    "Return mileage cannot be lower than pickup mileage."
                )
        ...
        for field_name in ("driver_license", "national_id"):
            uploaded = getattr(self, field_name, None)
            if uploaded and hasattr(uploaded, "name"):
                extension = Path(uploaded.name).suffix.lower()
                if extension not in allowed_document_extensions:
                    errors[field_name] = (
                        "Unsupported document format. Use PDF, JPG, PNG or WEBP."
                    )
                if getattr(uploaded, "size", 0) > max_document_size:
                    errors[field_name] = (
...
class RentalInspection(models.Model):
    """Immutable operational inspection record for vehicle handover/return."""
    ...
    class Meta:
        ordering = ["inspection_type", "-created_at"]
        constraints = [
            models.UniqueConstraint(
                fields=["booking", "inspection_type"],
                name="unique_booking_inspection_type",
            )
        ]

    def clean(self):
        errors = {}
        if self.inspection_type == "Return":
            pickup = self.booking.inspections.filter(inspection_type="Pickup").first()
            if pickup and self.mileage < pickup.mileage:
                errors["mileage"] = "Return mileage cannot be lower than pickup mileage."
        if self.damage_found and not self.damage_description.strip():
            errors["damage_description"] = "Describe the damage found during inspection."
        if errors:
            raise ValidationError(errors)

    def __str__(self):
        return f"Booking #{self.booking_id} - {self.inspection_type} inspection"


class ExtraCharge(models.Model):
    """Operational charges discovered during rental return inspection."""
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="rental_extra_charges",
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def clean(self):
        if self.amount is None or self.amount <= Decimal("0"):
            raise ValidationError({"amount": "Extra charge amount must be greater than zero."})
        if not self.description.strip():
            raise ValidationError({"description": "A description is required."})

    def __str__(self):
        return f"Booking #{self.booking_id} - {self.charge_type}: {self.amount}"
